...
def delete_useless_file(path):
    for child_name in os.listdir(path):
        if (child_path := Path(path, child_name)).is_file():
...
def get_filename_map_same_part_removal(root_path):
    ...
    for video_name in os.listdir(root_path):
        if not (video_path := Path(root_path, video_name)).is_dir():
            continue
        delete_useless_file(video_path)
        for season in os.listdir(video_path):
            if (not (season_path := Path(video_path, season)).is_dir()) or ("Season" not in season and "season" not in season):
                continue
...
if __name__ == "__main__":
    if not (root_path := Path(get_self_dir()[1], "rename_media")).exists():
        os.mkdir(root_path)
