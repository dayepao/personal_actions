...
@nb.njit
def newmark_beta_deprecated(wave, am, xi, T, beta=1/6, gamma=1/2):
...
@nb.njit
def newmark_beta(wave, am, xi, T, beta=1/6, gamma=1/2):
    """newmark-beta 积分求解单自由度结构地震荷载下的响应
    :param wave: 地震时程
    :param am: 时程波加速度峰值(m/s^2)
    :param xi: 阻尼比
...
def get_res(wave, am, xi, T):
    ...
    :param T: 结构自振周期
    """
    u_r, v_r, a = nigam_jennings(wave, am, xi, T)
    U_R = np.max(np.abs(u_r))  # 结构最大位移
    V_R = np.max(np.abs(v_r))  # 结构最大速度
    A = np.max(np.abs(a))  # 结构最大加速度
    return U_R, V_R, A
