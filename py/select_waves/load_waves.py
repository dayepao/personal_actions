...
def load_waves(wave_files):
    ...
    for wave_file in wave_files:
        ...
        T = np.arange(0, len(wave_data) * wave_file[1], wave_file[1])  # 时程时间
        waves.append((T, wave_data, wave_file[2] if len(wave_file) == 3 else os.path.splitext(os.path.basename(wave_file[0]))[0]))
    return waves
