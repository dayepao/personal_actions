...
for rsn_err in range(4577, 4840):
    try:
        rsn_list.remove(str(rsn_err))
    except ValueError:
        pass
# print(len(rsn_list))
# exit()
...
for file in file_path.iterdir():
    ...
    if not direction:
        direction = re.search(r".*(E|N|W|S)\.AT2", file.name)
    if not direction:
        direction = re.search(r".*(EW|NS)\.AT2", file.name)
    if not direction:
        direction = re.search(r".*(L|T|L1|T1|LN|TR)\.AT2", file.name)
    if not direction:
        direction = re.search(r".*(-90|360)\.AT2", file.name)
...
for rsn, items in result.items():
    unprocessed_files = [item for item in items if item["direction"] not in [None]]
