...
def gen_code_res_gb50011(alpha_max, Tg, xi, Ts):
    """生成《建筑抗震设计规范》(GB 50011-2010) 中的反应谱
...
def gen_code_res_gbt51408(alpha_max, Tg, xi, Ts):
    """生成《建筑隔震设计标准》(GB/T 51408-2021) 中的反应谱
    :param alpha_max: 地震影响系数最大值
    :param Tg: 场地特征周期
    :param xi: 阻尼比
    :param Ts: 谱周期序列
    ...
    # 生成规范反应谱
    alpha = np.zeros_like(Ts)
    alpha[Ts <= 0.1] = np.interp(Ts[Ts <= 0.1], [0, 0.1], [0.45 * alpha_max, eta * alpha_max])
    alpha[(Ts > 0.1) & (Ts <= Tg)] = eta * alpha_max
    alpha[(Ts > Tg) & (Ts <= 6)] = (Tg / Ts[(Ts > Tg) & (Ts <= 6)]) ** gamma * eta * alpha_max
